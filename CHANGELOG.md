...
- Миграция legado-ID (`gold→GLD`, `denarius→RAS`, `crown→VLR`) логируется только при фактическом переносе строк (идемпотентный тихий прогон).
- `MessagesConfig`: убран deprecated `ChatColor` (собственный colorize); отсутствующий ключ messages.yml даёт одноразовый warning с именем ключа вместо технической строки игроку.

### Изменено
- Версия 1.0.0 → 1.0.1 (pom.xml, plugin.yml).
