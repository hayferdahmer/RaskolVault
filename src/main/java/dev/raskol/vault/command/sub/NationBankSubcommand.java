...
/**
 * /rv bank … (FIX 1.1.1.2): добавлен import java.util.Map для Map.of(...).
 */
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        switch (op) {
            case "info" -> {
                ...
                if (national != null) {
                    send(sender, "&7 Покрытие: &f"
                            + String.format(Locale.ROOT, "%.1f%%",
                            bank.coverageOf(nation, national.id()) * 100.0D));
                    send(sender, "&7 Цена: &f"
                            + String.format(Locale.ROOT, "%.4f", bank.priceOf(national)) + " GLD");
                    send(sender, "&7 Паритет: &f"
                            + String.format(Locale.ROOT, "%.2f", bank.parityOf(nation))
                            + " &7· Налог: &f"
                            + String.format(Locale.ROOT, "%.1f%%", bank.taxOf(nation) * 100.0D));
                }
            }
            case "deposit", "withdraw" -> {
                ...
                send(sender, plugin.getMessages().prefix() + (ok
                        ? "&a" + (op.equals("deposit") ? "Внесено" : "Выведено") + " "
                        + Formatter.amount(amount, 2)
            ...
            case "parity", "tax" -> {
                ...
                boolean ok = op.equals("parity")
                        ? bank.setParity(nation, value)
                        : bank.setTax(nation, value);
                send(sender, plugin.getMessages().prefix() + (ok
                        ? "&a" + (op.equals("parity") ? "Паритет" : "Налог")
                        + " установлен: " + String.format(Locale.ROOT, "%.2f", value)
                        : "&c✖ Значение вне границ"));
            }
            default -> send(sender, plugin.getMessages().prefix()
                    + "&f/rv bank info|deposit|withdraw|parity|tax");
        }
    }

    private Currency nationalOf(String nation) {
        for (Currency cur : plugin.getCurrencies().all()) {
            if (cur.type() == CurrencyType.NATIONAL
